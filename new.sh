...
initial_ip=" AE94:32B1:5520:AB72::1/64"
...
do
  ip_suffix=`printf "%x\n" $i`
  ip_addr=" AE94:32B1:5520:AB72${ip_suffix}::1/64"  > /dev/null
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "ip addr add AE94:32B1:5520:AB72::1/64 dev azumi" >> /etc/private.sh
        ip_addr=" AE94:32B1:5520:AB7${ip_suffix}::1/64"
...
display_notification $'\e[93mAdding cron job for server!\e[0m'
	...
	ping -c 2 AE94:32B1:5520:AB72::2 | sed "s/.*/\x1b[94m&\x1b[0m/" 
	sleep 1
...
# IPv6 address
ip_address=" AE94:32B1:5520:AB72::2"
# maximum number
max_pings=4
# Interval
interval=60
...
do
    ip_suffix=`printf "%x\n" $i`
    ip_addr=" AE94:32B1:5520:AB7${ip_suffix}::1"
    echo "+---------------------------+"
...
# iran initial IP address
initial_ip=" AE94:32B1:5520:AB72::2/64"
ip addr add $initial_ip dev azumi > /dev/null
...
do
  ip_suffix=`printf "%x\n" $i`
  ip_addr=" AE94:32B1:5520:AB7${ip_suffix}::2/64" > /dev/null
...
# private.sh
    ...
    echo "ip tunnel add azumi mode sit remote $remote_ip local $local_ip ttl 255" >> /etc/private.sh
    echo "ip link set dev azumi up" >> /etc/private.sh
    echo "ip addr add AE94:32B1:5520:AB72::2/64 dev azumi" >> /etc/private.sh
        ip_addr=" AE94:32B1:5520:AB7${ip_suffix}::2/64"
        echo "ip addr add $ip_addr dev azumi" >> /etc/private.sh
	...
	ping -c 2 AE94:32B1:5520:AB72::1 | sed "s/.*/\x1b[94m&\x1b[0m/" 
	sleep 1
	display_notification $'\e[93mConfiguring keepalive service..\e[0m'
...
script_content='#!/bin/bash
# iPv6 address
ip_address=" AE94:32B1:5520:AB72::1"
max_pings=3
# interval
interval=50
# loop run
...
do
    ip_suffix=`printf "%x\n" $i`
    ip_addr=" AE94:32B1:5520:AB7${ip_suffix}::2"
    echo "+---------------------------+"
    echo -e "| \e[92m$ip_addr    \e[0m|"
done
echo "+---------------------------+"
